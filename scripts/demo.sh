...
admitted="$(curl --fail --silent --show-error -H 'content-type: application/json' --data-binary "@$plan" "$api/v1/plans")"
...
for _ in $(seq 1 60); do
  current="$(curl --fail --silent --show-error "$api/v1/runs/$run_id")"
  status="$(printf '%s' "$current" | jq -r '.status')"
  if [[ "$status" =~ ^(completed|manual_review|denied|failed)$ ]]; then
    printf '%s\n' "$current" | jq
    curl --fail --silent --show-error "$api/v1/runs/$run_id/events" | jq
    exit 0
  fi
  sleep 1
done
