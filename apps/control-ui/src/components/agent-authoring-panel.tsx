...
const emptyAgent = { id: "", name: "", description: "", role: "", prompt: "", modelProfileId: "model.standard.v1", runtimeTarget: "local_http" as "local_http" | "azure_foundry", skillIds: [] as string[] };
...
export function AgentAuthoringPanel({ agents, skills, profiles, busy, onBusy, onFeedback, onRefresh }: {
  ...
  const createAgent = async () => {
    onBusy(true);
    try {
      const response = await fetch("/v1/authoring/agents", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...agentForm, inputSchema: { type: "object" }, outputSchema: { type: "object" }, source: { type: "manual" }, version: "0.1.0", status: "active" }) });
