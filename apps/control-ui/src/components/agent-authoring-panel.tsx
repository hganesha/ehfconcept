...
const emptyAgent = { id: "", name: "", description: "", role: "", prompt: "", modelProfileId: "model.standard.v1", runtimeTarget: "local_http" as "local_http" | "azure_foundry", skillIds: [] as string[], caseWrites: "[]", inputSchema: emptyObjectSchema, outputSchema: emptyObjectSchema };
...
export function AgentAuthoringPanel({ agents, skills, profiles, busy, onBusy, onFeedback, onRefresh }: {
  ...
  const createAgent = async () => {
    ...
    try {
      ...
      const inputSchema = JSON.parse(agentForm.inputSchema) as unknown;
      const outputSchema = JSON.parse(agentForm.outputSchema) as unknown;
      if (!inputSchema || typeof inputSchema !== "object" || Array.isArray(inputSchema)) throw new Error("Input schema must be a JSON object.");
      if (!outputSchema || typeof outputSchema !== "object" || Array.isArray(outputSchema)) throw new Error("Output schema must be a JSON object.");
      const response = await fetch("/v1/authoring/agents", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...agentForm, caseWrites, inputSchema, outputSchema, source: { type: "manual" }, version: "0.1.0", status: "active" }) });
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <section className="card card-flush h-fit">
        ...
        {agentOpen ? <div className="border-y border-line bg-surface-2 p-4"><h3 className="text-xs font-bold">New agent contract</h3><div className="mt-3 grid gap-3 md:grid-cols-2">{([['id','Agent ID','agent.kyc.reviewer.v1'],['name','Name','KYC reviewer'],['description','Description','Reviews screening evidence'],['role','Role','Senior KYC analyst']] as const).map(([key,label,placeholder]) => <label key={key} className="block text-[11px] font-semibold">{label}<input className="field mt-1" value={agentForm[key]} onChange={(event) => setAgentForm({ ...agentForm, [key]: event.target.value })} placeholder={placeholder} /></label>)}<label className="block text-[11px] font-semibold">Model profile<select className="field mt-1" value={agentForm.modelProfileId} onChange={(event) => setAgentForm({ ...agentForm, modelProfileId: event.target.value })}>{profiles.map((profile) => <option key={profile.tierId} value={profile.tierId}>{profile.tierId} · {profile.resolvedOpenRouterModel}</option>)}</select></label><label className="block text-[11px] font-semibold">Runtime target<select className="field mt-1" value={agentForm.runtimeTarget} onChange={(event) => setAgentForm({ ...agentForm, runtimeTarget: event.target.value as "local_http" | "azure_foundry" })}><option value="local_http">Local LangGraph runtime</option><option value="azure_foundry">Azure Foundry hosted agent</option></select></label></div><label className="mt-3 block text-[11px] font-semibold">System prompt<textarea className="field mt-1 min-h-28 resize-y" value={agentForm.prompt} onChange={(event) => setAgentForm({ ...agentForm, prompt: event.target.value })} placeholder="Review the supplied evidence and return..." /></label><div className="mt-3 grid gap-3 md:grid-cols-2"><label className="block text-[11px] font-semibold">Input parameters (JSON Schema)<textarea className="field mono mt-1 min-h-44 resize-y" spellCheck={false} value={agentForm.inputSchema} onChange={(event) => setAgentForm({ ...agentForm, inputSchema: event.target.value })} /></label><label className="block text-[11px] font-semibold">Output parameters (JSON Schema)<textarea className="field mono mt-1 min-h-44 resize-y" spellCheck={false} value={agentForm.outputSchema} onChange={(event) => setAgentForm({ ...agentForm, outputSchema: event.target.value })} /></label></div><label className="mt-3 block text-[11px] font-semibold">Default case writes / case output parameters (JSON array)<textarea className="field mono mt-1 min-h-36 resize-y" spellCheck={false} value={agentForm.caseWrites} onChange={(event) => setAgentForm({ ...agentForm, caseWrites: event.target.value })} placeholder='[{"commandType":"SubmitDecisionRecommendation","when":"success","payload":{},"payloadSchema":{"type":"object"}}]' /></label>{skills.length ? <fieldset className="mt-3"><legend className="text-[11px] font-semibold">Skills</legend><div className="mt-2 flex flex-wrap gap-2">{skills.map(({ skill }) => <label key={skill.id} className="chip cursor-pointer"><input type="checkbox" checked={agentForm.skillIds.includes(skill.id)} onChange={(event) => setAgentForm({ ...agentForm, skillIds: event.target.checked ? [...agentForm.skillIds, skill.id] : agentForm.skillIds.filter((id) => id !== skill.id) })} />{skill.name}</label>)}</div></fieldset> : null}<p className="mt-3 text-[10.5px] text-ink-3">Runtime target, I/O schemas, and default case-write contracts are persisted into workflow.yaml when the agent is attached.</p><button className="btn btn-primary mt-3" disabled={busy || profiles.length === 0 || !profiles.some((profile) => profile.tierId === agentForm.modelProfileId) || !agentForm.id || !agentForm.name || !agentForm.description || !agentForm.role || !agentForm.prompt} onClick={() => void createAgent()}><Bot className="h-3.5 w-3.5" />Register agent</button></div> : null}
        <div className="divide-y divide-line">{agents.map(({ agent }) => { const profile = profiles.find((item) => item.tierId === agent.modelProfileId); return <div key={agent.id} className="p-4"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-bold">{agent.name}</span><span className={`chip ${profile ? "" : "chip-danger"}`}>{agent.modelProfileId}</span><span className="chip">{agent.runtimeTarget === "azure_foundry" ? "Azure Foundry" : "Local LangGraph"}</span><span className="chip chip-ok">{agent.status}</span></div><p className="mt-1.5 text-xs text-ink-2">{agent.description}</p><p className="mono mt-2 text-[10.5px] text-ink-3">{agent.id} · {agent.skillIds.length} skills · {agent.caseWrites.length} case writes</p></div>; })}{agents.length === 0 ? <p className="p-6 text-center text-xs text-ink-3">Use + Create agent or import a repository.</p> : null}</div>
      </section>
