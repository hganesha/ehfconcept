...
`make demo` is the fast invoice smoke test. `make demo-kyc-e2e` exercises the richer pilot path across both authorities: the compiled LangGraph harness resolves policy, performs bounded public-source investigation, captures evidence and findings, and submits a model recommendation. The script then records independent QA and human review, runs the final gate, and creates the effective disposition. It prints the run ID, trace ID, case ID, provider mode, final case state, and ledger verification result.
...
| `EDGE_TOKEN_SCOPE` / `INTERNAL_API_TOKEN_SCOPE` | unused | required managed-identity audiences |
